...
while cnt ~= 0  % Schleife ueber alle VPs (All Subjects)
    ...
    if cnt ~= 0
        ...
        for t = 1:size(tab,1) % Schleife ueber alle Trials der VP (All Trials per Subject)
            ...
            block           = tab(t,1);
            trial           = tab(t,2);            
            tFixOnEDF       = tab(t,11); %trial start
            tEDFpreCueOn    = tFixOnEDF+tab(t,12);    
            tEDFpreISIOn    = tFixOnEDF+tab(t,13);
            tEDFstimOn      = tFixOnEDF+tab(t,14);
            tEDFstimOff     = tFixOnEDF+tab(t,15);
            tEDFrespTone    = tFixOnEDF+tab(t,16);
 %           tEDFClr         = tFixOnEDF+tab(t,17);
            ...
            if sacReq == 1
                idxrs = find(dat(:,1)>=tFixOnEDF & dat(:,1)<=tEDFrespTone);%response interval    
            end
            ...
            if length(timers)<length(tFixOnEDF:SAMPRATE/samrat:tEDFrespTone) %bl   
                loss = 1;
                ex.loss = ex.loss + 1;
            else
                ...
                if plotData
                    ...
                    axes(ax(2));
                    plot(timers-tFixOnEDF,vrsf(:,1),'k-','color',[0.6 0.6 0.6]); %was tProOnEDF for MR
                    hold on;
                    plot(timers-tFixOnEDF,vrs(:,1),'g-'); %timers = tFixOnEDF = 0:450
                    for i = 1:size(mrs,1)
                        plot(timers(mrs(i,1):mrs(i,2))-tFixOnEDF,vrs(mrs(i,1):mrs(i,2),1)','r-','linewidth',2);
                    end
                    hold off;
                    xlim(timers([1 end])-tFixOnEDF);
                    ylim([-1000 1000]);
                    
                    %Plot filtered and unfiltered vertical velocity traces
                    axes(ax(3));
                    plot(timers-tFixOnEDF,vrsf(:,2),'k-','color',[0.6 0.6 0.6]);
                    hold on;
                    plot(timers-tFixOnEDF,vrs(:,2),'g-');
                    for i = 1:size(mrs,1)
                        plot(timers(mrs(i,1):mrs(i,2))-tFixOnEDF,vrs(mrs(i,1):mrs(i,2),2)','r-','linewidth',2);
                    end
                    hold off;
                    xlim(timers([1 end])-tFixOnEDF);
                    ylim([-1000 1000]);
            ...
            if sacReq == 1 && ~loss
                for rs = 1:nRS % (for each microsaccade detected...)
                    ...
                    sacOnset   = timers(mrs(reaSacNumber,1))-tFixOnEDF; % start-screen-geloggt
                    sacOffset  = timers(mrs(reaSacNumber,2))-tFixOnEDF; % start-screen-geloggt
